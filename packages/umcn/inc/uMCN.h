...
#define MCN_MALLOC(size)            cd_malloc(size)
#define MCN_FREE(ptr)               cd_free(ptr)
#define MCN_ENTER_CRITICAL          CD_ENTER_CRITICAL()
#define MCN_EXIT_CRITICAL           CD_EXIT_CRITICAL()
#define MCN_EVENT_HANDLE            event_id_t//rt_sem_t
#define MCN_SEND_EVENT(event)       //rt_sem_release(event)
#define MCN_WAIT_EVENT(event, time) //rt_sem_take(event, time)
#define MCN_ASSERT(EX)              CD_ASSERT(EX)
...
struct mcn_hub {
    const char* obj_name;
    const cd_uint32_t obj_size;
    void* pdata;
    McnNode_t link_head;
    McnNode_t link_tail;
    cd_uint32_t link_num;
    cd_uint8_t published;
    cd_uint8_t suspend;
    int (*echo)(void* parameter);
    /* publish freq estimate */
    float freq;
    cd_uint16_t freq_est_window[MCN_FREQ_EST_WINDOW_LEN];
    cd_uint16_t window_index;
};
